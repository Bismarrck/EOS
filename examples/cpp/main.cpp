...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1) {
    ...
    std::cout << "No path provided, using default: " << data_dir_path
              << std::endl;
  }

  EquationOfStateV1 eos_manager;  // Using namespace if header uses it

  std::vector<int> eos_ids_to_load = {
      1, 10000};  // Load analytic air and one complicated

  int istat = eos_manager.initialize(eos_ids_to_load, data_dir_path);
  if (istat != 0) {  // Assuming 0 is EOS_SUCCESS
    std::cerr << "Failed to initialize EOS manager. Error: " << istat
              << std::endl;
    return 1;
  }
  ...
  print_results("Analytic Air (ID 1)", istat, result.P, result.E);

  // Test Complicated (ID 10000)
  rho = 1.8;
  T = 1200.0;
  result = eos_manager.compute(10000, rho, T);
  print_results("Complicated EOS (ID 10000)", istat, result.P, result.E);
